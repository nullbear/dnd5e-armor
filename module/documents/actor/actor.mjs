...
export default class Actor5e extends SystemDocumentMixin(Actor) {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("dnd5a", "isPolymorphed") || false;
  }
  ...
  _initializeSource(source, options={}) {
    ...
    if ( !source._id || !options.pack || dnd5a.moduleArt.suppressArt ) return source;
    const uuid = `Compendium.${options.pack}.${source._id}`;
    const art = game.dnd5a.moduleArt.map.get(uuid);
  ...
  /** @inheritDoc */
  prepareData() {
    if ( this.system.modelProvider !== dnd5a ) return super.prepareData();
  ...
  *allApplicableEffects() {
    for ( const effect of super.allApplicableEffects() ) {
      if ( (effect.getFlag("dnd5a", "type") !== "enchantment") && !effect.getFlag("dnd5a", "rider") ) yield effect;
    }
  }
  ...
  /** @inheritDoc */
  prepareDerivedData() {
    const origin = this.getFlag("dnd5a", "summon.origin");
    ...
    if ( (this.system.modelProvider !== dnd5a) || (this.type === "group") ) return;
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.DND5A.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.DND5A.CR_EXP_LEVELS[cr];
  }
   ...
   * @param {string} key      A key in `DND5A.conditionEffects`.
   * @returns {boolean}       Whether the actor is affected.
   */
  hasConditionEffect(key) {
    const props = CONFIG.DND5A.conditionEffects[key] ?? new Set();
  ...
  _prepareScaleValues() {
    this.system.scale = this.items.reduce((scale, item) => {
      if ( CONFIG.DND5A.advancementTypes.ScaleValue.validItemTypes.has(item.type) ) {
   ...
   *                                             `this.flags.dnd5a.originalActor` are used.
  ...
  _prepareSkill(skillId, {
    ...
    const feats = CONFIG.DND5A.characterFlags;
  ...
  _prepareArmorClass() {
    ...
    const armorTypes = new Set(Object.keys(CONFIG.DND5A.armorTypes));
      ...
      default:
        ...
        try {
          const replaced = replaceFormulaData(formula, rollData, {
            actor: this, missing: null, property: game.i18n.localize("DND5A.ArmorClass")
          });
          ...
          this._preparationWarnings.push({
            message: game.i18n.format("DND5A.WarnBadACFormula", { formula }), link: "armor", type: "error"
          });
          const replaced = Roll.replaceFormulaData(CONFIG.DND5A.armorClasses.default.formula, rollData);
    ...
    if ( shields.length ) {
      if ( shields.length > 1 ) this._preparationWarnings.push({
        message: game.i18n.localize("DND5A.WarnMultipleShields"), type: "warning"
      });
  ...
  _prepareInitiative(bonusData, globalCheckBonus=0) {
    ...
    const abilityId = init.ability || CONFIG.DND5A.defaultAbilities.initiative;
  ...
  _prepareSpellcasting() {
    ...
    for ( const type of Object.keys(CONFIG.DND5A.spellcastingTypes) ) {
  ...
  static computeClassProgression(progression, cls, {actor, spellcasting, count=1}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `dnd5a.computeLeveledProgression`).
     * @param {object} progression                    Spellcasting progression data. *Will be mutated.*
     ...
     * @function dnd5a.computeSpellcastingProgression
     * @memberof hookEvents
     */
    const allowed = Hooks.call(
      `dnd5a.compute${type.capitalize()}Progression`, progression, actor, cls, spellcasting, count
    );
  ...
  static computeLeveledProgression(progression, actor, cls, spellcasting, count) {
    const prog = CONFIG.DND5A.spellcastingTypes.leveled.progression[spellcasting.progression];
    if ( !prog ) return;
  ...
  static prepareSpellcastingSlots(spells, type, progression, {actor}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `dnd5a.prepareLeveledSlots`).
     * @param {object} spells        The `data.spells` object within actor's data. *Will be mutated.*
     * @param {Actor5e} actor        Actor for whom the data is being prepared.
     * @param {object} progression   Spellcasting progression data.
     * @returns {boolean}            Explicitly return false to prevent default preparation from being performed.
     * @function dnd5a.prepareSpellcastingSlots
     * @memberof hookEvents
     */
    const allowed = Hooks.call(`dnd5a.prepare${type.capitalize()}Slots`, spells, actor, progression);
  ...
  static prepareLeveledSlots(spells, actor, progression) {
    const levels = Math.clamp(progression.slot, 0, CONFIG.DND5A.maxLevel);
    const slots = CONFIG.DND5A.SPELL_SLOT_TABLE[Math.min(levels, CONFIG.DND5A.SPELL_SLOT_TABLE.length) - 1] ?? [];
    for ( const level of Array.fromRange(Object.keys(CONFIG.DND5A.spellLevels).length - 1, 1) ) {
  ...
  static prepareAltSlots(spells, actor, progression, key, table) {
    ...
    let keyLevel = Math.clamp(progression[key], 0, CONFIG.DND5A.maxLevel);
    spells[key] ??= {};
  ...
  static preparePactSlots(spells, actor, progression) {
    this.prepareAltSlots(spells, actor, progression, "pact", CONFIG.DND5A.pactCastingProgression);
  }
  ...
  async _preCreate(data, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      const size = CONFIG.DND5A.actorSizes[this.system.traits.size || "med"].token ?? 1;
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      ...
      if ( newSize && (newSize !== this.system.traits?.size) ) {
        let size = CONFIG.DND5A.actorSizes[newSize].token ?? 1;
    ...
    if ( "hp" in (this.system.attributes || {}) ) {
      ...
      foundry.utils.setProperty(options, "dnd5a.hp", { ...this.system.attributes.hp });
    }

    // Record previous exhaustion level.
    if ( Number.isFinite(foundry.utils.getProperty(changed, "system.attributes.exhaustion")) ) {
      foundry.utils.setProperty(options, "dnd5a.originalExhaustion", this.system.attributes.exhaustion);
    }
  }
  ...
  async applyDamage(damages, options={}) {
     ...
     * @function dnd5a.preApplyDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("dnd5a.preApplyDamage", this, amount, updates, options) === false ) return this;
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @function dnd5a.applyDamage
     * @memberof hookEvents
     */
    Hooks.callAll("dnd5a.applyDamage", this, amount, options);
  ...
  calculateDamage(damages, options={}) {
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @returns {boolean}                         Explicitly return `false` to prevent damage application.
     * @function dnd5a.preCalculateDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("dnd5a.preCalculateDamage", this, damages, options) === false ) return false;
    ...
    const hasEffect = (category, type, properties) => {
      ...
      if ( !CONFIG.DND5A.damageTypes[type]?.isPhysical || !properties?.size ) return true;
      return !config.bypasses?.intersection(properties)?.size;
    };

    const skipped = type => {
      if ( options.only === "damage" ) return type in CONFIG.DND5A.healingTypes;
      if ( options.only === "healing" ) return type in CONFIG.DND5A.damageTypes;
      return false;
    };
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @returns {boolean}                         Explicitly return `false` to prevent damage application.
     * @function dnd5a.calculateDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("dnd5a.calculateDamage", this, damages, options) === false ) return false;
  ...
  async beginConcentrating(item, effectData={}) {
     ...
     * @function dnd5a.preBeginConcentrating
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("dnd5a.preBeginConcentrating", this, item, effectData) === false ) return;
  ...
  async endConcentration(target) {
    ...
    if ( foundry.utils.getType(target) === "string" ) effect = effects.find(e => e.id === target);
      ...
      effect = effects.find(e => {
        const data = e.getFlag("dnd5a", "itemData") ?? {};
     ...
     * @function dnd5a.preEndConcentration
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("dnd5a.preEndConcentration", this, effect) === false) return [];

    await effect.delete();

    /**
     * A hook that is called after a concentration effect is deleted.
     * @function dnd5a.endConcentration
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor ending concentration.
     * @param {ActiveEffect5e} effect     The ActiveEffect that was deleted.
     */
    Hooks.callAll("dnd5a.endConcentration", this, effect);
  ...
  async challengeConcentration({ dc=10, ability=null }={}) {
    ...
    if ( ability in CONFIG.DND5A.abilities ) dataset.ability = ability;
    ...
    return ChatMessage.implementation.create({
      content: await renderTemplate("systems/dnd5a/templates/chat/request-card.hbs", {
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("dnd5a", "remarkableAthlete")
      && CONFIG.DND5A.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  async rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("dnd5a", "reliableTalent"));

    // Roll and return
    const flavor = game.i18n.format("DND5A.SkillPromptTitle", {skill: CONFIG.DND5A.skills[skillId]?.label ?? ""});
    const rollData = foundry.utils.mergeObject({
      data: data,
      title: `${flavor}: ${this.name}`,
      flavor,
      chooseModifier: true,
      halflingLucky: this.getFlag("dnd5a", "halflingLucky"),
      reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.dnd5a.roll": {type: "skill", skillId }
      }
    }, options);
     ...
     * @function dnd5a.preRollSkill
     * @memberof hookEvents
     * @param {Actor5e} actor                Actor for which the skill check is being rolled.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     * @param {string} skillId               ID of the skill being rolled as defined in `DND5A.skills`.
     * @returns {boolean}                    Explicitly return `false` to prevent skill check from being rolled.
     */
    if ( Hooks.call("dnd5a.preRollSkill", this, rollData, skillId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after a skill check has been rolled for an Actor.
     * @function dnd5a.rollSkill
     * @memberof hookEvents
     * @param {Actor5e} actor   Actor for which the skill check has been rolled.
     * @param {D20Roll} roll    The resulting roll.
     * @param {string} skillId  ID of the skill that was rolled as defined in `DND5A.skills`.
     */
    if ( roll ) Hooks.callAll("dnd5a.rollSkill", this, roll, skillId);
  ...
  async rollToolCheck(toolId, options={}) {
    ...
    const reliableTalent = (prof?.multiplier >= 1 && this.getFlag("dnd5a", "reliableTalent"));

    const flavor = game.i18n.format("DND5A.ToolPromptTitle", {tool: Trait.keyLabel(toolId, {trait: "tool"}) ?? ""});
    const rollData = foundry.utils.mergeObject({
      data, flavor,
      title: `${flavor}: ${this.name}`,
      chooseModifier: true,
      ...
      messageData: {
        ...
        "flags.dnd5a.roll": {type: "tool", toolId}
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @returns {boolean}                    Explicitly return `false` to prevent skill check from being rolled.
     */
    if ( Hooks.call("dnd5a.preRollToolCheck", this, rollData, toolId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after a tool check has been rolled for an Actor.
     * @function dnd5a.rollTool
     * @memberof hookEvents
     ...
     */
    if ( roll ) Hooks.callAll("dnd5a.rollToolCheck", this, roll, toolId);
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.DND5A.abilities[abilityId]?.label ?? "";
    new Dialog({
      title: `${game.i18n.format("DND5A.AbilityPromptTitle", {ability: label})}: ${this.name}`,
      content: `<p>${game.i18n.format("DND5A.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("DND5A.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("DND5A.ActionSave"),
  ...
  async rollAbilityTest(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("DND5A.AbilityPromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.dnd5a.roll": {type: "ability", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @function dnd5a.preRollAbilityTest
     * @memberof hookEvents
     * @param {Actor5e} actor                Actor for which the ability test is being rolled.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     * @param {string} abilityId             ID of the ability being rolled as defined in `DND5A.abilities`.
     * @returns {boolean}                    Explicitly return `false` to prevent ability test from being rolled.
     */
    if ( Hooks.call("dnd5a.preRollAbilityTest", this, rollData, abilityId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after an ability test has been rolled for an Actor.
     * @function dnd5a.rollAbilityTest
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the ability test has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     * @param {string} abilityId  ID of the ability that was rolled as defined in `DND5A.abilities`.
     */
    if ( roll ) Hooks.callAll("dnd5a.rollAbilityTest", this, roll, abilityId);
  ...
  async rollAbilitySave(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("DND5A.SavePromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.dnd5a.roll": {type: "save", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @function dnd5a.preRollAbilitySave
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("dnd5a.preRollAbilitySave", this, rollData, abilityId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after an ability save has been rolled for an Actor.
     * @function dnd5a.rollAbilitySave
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the ability save has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     * @param {string} abilityId  ID of the ability that was rolled as defined in `DND5A.abilities`.
     */
    if ( roll ) Hooks.callAll("dnd5a.rollAbilitySave", this, roll, abilityId);
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.system.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3) ) {
      ui.notifications.warn("DND5A.DeathSaveUnnecessary", {localize: true});
      return null;
    }
    ...
    if ( this.getFlag("dnd5a", "diamondSoul") ) {
      parts.push("@prof");
    ...
    // Evaluate the roll
    const flavor = game.i18n.localize("DND5A.DeathSavingThrow");
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
     ...
     * @function dnd5a.preRollDeathSave
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("dnd5a.preRollDeathSave", this, rollData) === false ) return;
    ...
    if ( roll.total >= (roll.options.targetValue ?? 10) ) {
      ...
      if ( roll.isCritical ) {
        ...
        details.chatString = "DND5A.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        details.chatString = "DND5A.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        details.chatString = "DND5A.DeathSaveFailure";
      }
    }
     ...
     */
    if ( Hooks.call("dnd5a.rollDeathSave", this, roll, details) === false ) return roll;
  ...
  async rollConcentration(options={}) {
     ...
     * @function dnd5a.preRollConcentration
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("dnd5a.preRollConcentration", this, options) === false ) return;
     ...
     * @function dnd5a.rollConcentration
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the saving throw has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     */
    if ( roll ) Hooks.callAll("dnd5a.rollConcentration", this, roll);
  ...
  getInitiativeRoll(options={}) {
    ...
    const data = this.getRollData();
    const flags = this.flags.dnd5a || {};
    if ( flags.initiativeAdv ) options.advantageMode ??= dnd5a.dice.D20Roll.ADV_MODE.ADVANTAGE;
    ...
    const tiebreaker = game.settings.get("dnd5a", "initiativeDexTiebreaker");
    ...
    options = foundry.utils.mergeObject({
      flavor: options.flavor ?? game.i18n.localize("DND5A.Initiative"),
  ...
  async rollInitiativeDialog(rollOptions={}) {
    ...
    const choice = await roll.configureDialog({
      ...
      title: `${game.i18n.localize("DND5A.InitiativeRoll")}: ${this.name}`,
      chooseModifier: false,
      defaultAction: rollOptions.advantageMode ?? dnd5a.dice.D20Roll.ADV_MODE.NORMAL
    });
  ...
  async rollInitiative(options={}, rollOptions={}) {
     ...
     * @function dnd5a.preRollInitiative
     * @memberof hookEvents
     * @param {Actor5e} actor  The Actor that is rolling initiative.
     * @param {D20Roll} roll   The initiative roll.
     */
    if ( Hooks.call("dnd5a.preRollInitiative", this, this._cachedInitiativeRoll) === false ) {
  ...
  async rollHitDie(denomination, options={}) {
    ...
    if ( this.type === "npc" ) {
      ...
      if ( !this.system.attributes.hd.value ) {
        ui.notifications.error(game.i18n.format("DND5A.HitDiceNPCWarn", {name: this.name}));
        return null;
      }
    }
    ...
    // Otherwise check classes
      ...
      if ( !cls ) {
        ui.notifications.error(game.i18n.format("DND5A.HitDiceWarn", {name: this.name, formula: denomination}));
        return null;
      }
    }

    // Prepare roll data
    const flavor = game.i18n.localize("DND5A.HitDiceRoll");
     ...
     */
    if ( Hooks.call("dnd5a.preRollHitDie", this, rollConfig, denomination) === false ) return;
     ...
     * @returns {boolean}               Explicitly return `false` to prevent updates from being performed.
     */
    if ( Hooks.call("dnd5a.rollHitDie", this, roll, updates) === false ) return roll;
   ...
   * @see {@link dnd5a.preRollClassHitPoints}
   */
  async rollClassHitPoints(item, { chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("DND5A.AdvancementHitPointsRollMessage", { class: item.name });
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.implementation.getSpeaker({ actor: this }),
      "flags.dnd5a.roll": { type: "hitPoints" }
    };

    /**
     * A hook event that fires before hit points are rolled for a character's class.
     * @function dnd5a.preRollClassHitPoints
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("dnd5a.preRollClassHitPoints", this, item, rollData, messageData);
     ...
     * @function dnd5a.rollClassHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("dnd5a.rollClassHitPoints", this, roll);
   ...
   * @param {boolean} [options.chatMessage=true]  Display the chat message for this roll.
   * @returns {Promise<Roll>}                     The completed roll.
   * @see {@link dnd5a.preRollNPCHitPoints}
   */
  async rollNPCHitPoints({ chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("DND5A.HPFormulaRollMessage");
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
     ...
     * @function dnd5a.preRollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor            Actor for which the hit points are being rolled.
     * @param {object} rollData
     * @param {string} rollData.formula  The string formula to parse.
     ...
     * @param {object} messageData       The data object to use when creating the message.
     */
    Hooks.callAll("dnd5a.preRollNPCHitPoints", this, rollData, messageData);
     ...
     * @function dnd5a.rollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("dnd5a.rollNPCHitPoints", this, roll);
  ...
  async shortRest(config={}) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: CONFIG.DND5A.restTypes.short.duration[game.settings.get("dnd5a", "restVariant")]
    }, config);

    /**
     * A hook event that fires before a short rest is started.
     * @function dnd5a.preShortRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("dnd5a.preShortRest", this, config) === false ) return;
  ...
  async longRest(config={}) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: CONFIG.DND5A.restTypes.long.duration[game.settings.get("dnd5a", "restVariant")]
    }, config);

    /**
     * A hook event that fires before a long rest is started.
     * @function dnd5a.preLongRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("dnd5a.preLongRest", this, config) === false ) return;
     ...
     * @function dnd5a.longRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being continued.
     */
    if ( Hooks.call("dnd5a.longRest", this, config) === false ) return;
  ...
  async _rest(config, result={}, ...args) {
     ...
     * @function dnd5a.preRestCompleted
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     ...
     */
    if ( Hooks.call("dnd5a.preRestCompleted", this, result, config) === false ) return result;
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    let restFlavor;
    switch (game.settings.get("dnd5a", "restVariant")) {
      case "normal":
        restFlavor = (longRest && newDay) ? "DND5A.LongRestOvernight" : `DND5A.${length}RestNormal`;
        break;
      case "gritty":
        restFlavor = (!longRest && newDay) ? "DND5A.ShortRestOvernight" : `DND5A.${length}RestGritty`;
        break;
      case "epic":
        restFlavor = `DND5A.${length}RestEpic`;
        break;
    }

    // Determine the chat message to display
    let message;
    if ( diceRestored && healthRestored ) message = `DND5A.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "DND5A.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "DND5A.LongRestResultHitDice";
    else message = `DND5A.${length}RestResultShort`;
    ...
    let chatData = {
      ...
      "flags.dnd5a.rest": { type: longRest ? "long" : "short" }
    };
  ...
  _getRestSpellRecovery({recoverShort=true, recoverLong=true}={}) {
    ...
    Object.entries(CONFIG.DND5A.spellPreparationModes).forEach(([k, v]) => {
      const isSR = CONFIG.DND5A.spellcastingTypes[k === "prepared" ? "leveled" : k]?.shortRest;
  ...
  async _getRestItemUsesRecovery({recoverShortRestUses=true, recoverLongRestUses=true,
    ...
    for ( let item of this.items ) {
      ...
      if ( recoverDailyUses && uses.recovery && CONFIG.DND5A.limitedUsePeriods[uses.per]?.formula ) {
        const roll = new Roll(uses.recovery, item.getRollData());
        if ( recoverLongRestUses && (game.settings.get("dnd5a", "restVariant") === "gritty") ) {
        ...
        try {
          ...
          ui.notifications.warn(game.i18n.format("DND5A.ItemRecoveryFormulaWarning", {
            name: item.name,
        ...
        if ( newValue !== uses.value ) {
          ...
          const locKey = `DND5A.Item${diff < 0 ? "Loss" : "Recovery"}Roll${isMax ? "Max" : ""}`;
  ...
  _prepareMovementAttribution() {
    ...
    const units = movement.units || Object.keys(CONFIG.DND5A.movementUnits)[0];
    return Object.entries(CONFIG.DND5A.movementTypes).reduce((html, [k, label]) => {
  ...
  async _prepareArmorClassAttribution({ title }={}) {
    ...
    const cfg = CONFIG.DND5A.armorClasses[ac.calc];
    const attribution = [];

    if ( ac.calc === "flat" ) {
      attribution.push({
        label: game.i18n.localize("DND5A.ArmorClassFlat"),
    ...
    switch ( ac.calc ) {
      ...
      case "natural":
        attribution.push({
          label: game.i18n.localize("DND5A.ArmorClassNatural"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: ac.flat
        });
        break;
      ...
      default:
        ...
        let label = game.i18n.localize("DND5A.PropertyBase");
        if ( armorInFormula ) label = this.armor?.name ?? game.i18n.localize("DND5A.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.shield?.name ?? game.i18n.localize("DND5A.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
    ...
    if ( ac.cover !== 0 ) attribution.push({
      label: game.i18n.localize("DND5A.Cover"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.cover
    });
  ...
  getOriginalStats() {
    ...
    if ( this.isPolymorphed ) {
      const transformOptions = this.flags.dnd5a?.transformOptions;
      const original = game.actors?.get(this.flags.dnd5a?.originalActor);
      if ( original ) {
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("dnd5a", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      ui.notifications.warn("DND5A.PolymorphWarn", {localize: true});
      return null;
    }

    // Get the original Actor data and the new source data
    const o = this.toObject();
    o.flags.dnd5a = o.flags.dnd5a || {};
    o.flags.dnd5a.transformOptions = {mergeSkills, mergeSaves};
    const source = target.toObject();

    if ( keepSelf ) {
      o.img = source.img;
      o.name = `${o.name} (${game.i18n.localize("DND5A.PolymorphSelf")})`;
    }
    ...
    if ( game.release.generation >= 12 ) d.prototypeToken.ring = source.prototypeToken.ring;
      ...
      foundry.utils.setProperty(d.prototypeToken, "flags.dnd5a.tokenRing", foundry.utils.mergeObject(
        foundry.utils.getProperty(d.prototypeToken, "flags.dnd5a.tokenRing") ?? {},
        foundry.utils.getProperty(source.prototypeToken, "flags.dnd5a.tokenRing") ?? {},
        { inplace: false }
      ));
    }
    ...
    if ( !keepSelf ) {
      ...
      for ( let k of Object.keys(abilities) ) {
        ...
        const type = CONFIG.DND5A.abilities[k]?.type;
      ...
      if ( !keepClass && d.system.details.cr ) {
        const cls = new dnd5a.dataModels.item.ClassData({levels: d.system.details.cr});
        d.items.push({
          type: "class",
          name: game.i18n.localize("DND5A.PolymorphTmpClass"),
          system: cls.toObject()
        });
      }
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.dnd5a.originalActor ) d.flags.dnd5a.originalActor = this.id;
    d.flags.dnd5a.isPolymorphed = true;

    // Gather previous actor data
    const previousActorIds = this.getFlag("dnd5a", "previousActorIds") || [];
    previousActorIds.push(this._id);
    foundry.utils.setProperty(d.flags, "dnd5a.previousActorIds", previousActorIds);
    ...
    if ( this.isToken ) {
      ...
      foundry.utils.setProperty(tokenData, "flags.dnd5a.previousActorData", previousActorData);
     ...
     * @param {object} [options]
     */
    Hooks.callAll("dnd5a.transformActor", this, target, d, {
    ...
    const updates = tokens.map(t => {
      ...
      const dOriginalActor = foundry.utils.getProperty(d, "flags.dnd5a.originalActor");
      foundry.utils.setProperty(newTokenData, "flags.dnd5a.originalActor", dOriginalActor);
      foundry.utils.setProperty(newTokenData, "flags.dnd5a.isPolymorphed", true);
      return newTokenData;
    });
  ...
  async revertOriginalForm({renderSheet=true}={}) {
    ...
    if ( !this.isOwner ) {
      ui.notifications.warn("DND5A.PolymorphRevertWarn", {localize: true});
      return null;
    }

    /**
     * A hook event that fires just before the actor is reverted to original form.
     * @function dnd5a.revertOriginalForm
     * @memberof hookEvents
     * @param {Actor} this                 The original actor before transformation.
     * @param {object} [options]
     */
    Hooks.callAll("dnd5a.revertOriginalForm", this, {renderSheet});
    const previousActorIds = this.getFlag("dnd5a", "previousActorIds") ?? [];
    const isOriginalActor = !previousActorIds.length;
    const isRendered = this.sheet.rendered;

    // Obtain a reference to the original actor
    const original = game.actors.get(this.getFlag("dnd5a", "originalActor"));
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("DND5A.PolymorphRevertNoOriginalActorWarn", {
          reference: this.getFlag("dnd5a", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = (await baseActor.getTokenDocument()).toObject();
      const actorData = this.token.getFlag("dnd5a", "previousActorData");
      ...
      if ( game.release.generation >= 12 ) tokenUpdate.ring = prototypeTokenData.ring;
        ...
        foundry.utils.setProperty(tokenUpdate, "flags.dnd5a.tokenRing", foundry.utils.mergeObject(
          foundry.utils.getProperty(tokenUpdate, "flags.dnd5a.tokenRing") ?? {},
          foundry.utils.getProperty(prototypeTokenData, "flags.dnd5a.tokenRing") ?? {},
          { inplace: false }
        ));
      }
      ...
      if ( isOriginalActor ) {
        await this.unsetFlag("dnd5a", "isPolymorphed");
        await this.unsetFlag("dnd5a", "previousActorIds");
        await this.token.unsetFlag("dnd5a", "previousActorData");
      }
    ...
    if ( canvas.ready ) {
      ...
      const tokenUpdates = tokens.map(t => {
        ...
        delete update.x;
        delete update.y;
        if ( (game.release.generation < 12) && !foundry.utils.getProperty(tokenData, "flags.dnd5a.tokenRing") ) {
          foundry.utils.setProperty(update, "flags.dnd5a.tokenRing", {});
        }
        return update;
      });
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: "DND5A.PolymorphRestoreTransformation",
      ...
      name: "DND5A.Group.Primary.Set",
      icon: '<i class="fa-solid fa-star"></i>',
      callback: li => {
        game.settings.set("dnd5a", "primaryParty", { actor: game.actors.get(li[0].dataset.documentId) });
      },
      condition: li => {
        const actor = game.actors.get(li[0].dataset.documentId);
        const primary = game.settings.get("dnd5a", "primaryParty")?.actor;
        return game.user.isGM && (actor.type === "group")
          && (actor.system.type.value === "party") && (actor !== primary);
      },
      group: "system"
    }, {
      name: "DND5A.Group.Primary.Remove",
      icon: '<i class="fa-regular fa-star"></i>',
      callback: li => {
        game.settings.set("dnd5a", "primaryParty", { actor: null });
      },
      condition: li => {
        const actor = game.actors.get(li[0].dataset.documentId);
        const primary = game.settings.get("dnd5a", "primaryParty")?.actor;
  ...
  static formatCreatureType(typeData) {
    ...
    } else if ( typeData.value in CONFIG.DND5A.creatureTypes ) {
      const code = CONFIG.DND5A.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? code.plural : code.label);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("DND5A.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.DND5A.actorSizes[typeData.swarm].label),
        type: localizedType
      });
    }
  ...
  async _onUpdate(data, options, userId) {
    ...
    if ( hp && !options.isRest && !options.isAdvancement ) {
      ...
      if ( Number.isInteger(changes.total) && (changes.total !== 0) ) {
        ...
        if ( !game.settings.get("dnd5a", "disableConcentration") && (userId === game.userId) && (changes.total < 0) ) {
         ...
         */
        Hooks.callAll(`dnd5a.${changes.total > 0 ? "heal" : "damage"}Actor`, this, changes, data, userId);
      }
    }
  }
  ...
  _displayTokenEffect(changes) {
    ...
    const fill = CONFIG.DND5A.tokenHPColors[key];
  ...
  async _onUpdateExhaustion(data, options) {
    ...
    if ( level < 1 ) return effect?.delete();
      ...
      const originalExhaustion = foundry.utils.getProperty(options, "dnd5a.originalExhaustion");
      return effect.update({ "flags.dnd5a.exhaustionLevel": level }, { dnd5a: { originalExhaustion } });
    } else {
      effect = await ActiveEffect.implementation.fromStatusEffect("exhaustion", { parent: this });
      effect.updateSource({ "flags.dnd5a.exhaustionLevel": level });
  ...
  updateEncumbrance(options) {
    ...
    if ( !encumbrance || (game.settings.get("dnd5a", "encumbrance") === "none") ) return;
    const statuses = [];
    const variant = game.settings.get("dnd5a", "encumbrance") === "variant";
    ...
    const effectData = { ...CONFIG.DND5A.encumbrance.effects[statuses[0]], statuses };
    if ( effect ) {
      const originalEncumbrance = effect.statuses.first();
      return effect.update(effectData, { dnd5a: { originalEncumbrance } });
    }
