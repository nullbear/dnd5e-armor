...
export default class ActorSheet5eNPC extends ActorSheet5e {
  ...
  _prepareItems(context) {
    ...
    const features = {
      weapons: { label: game.i18n.localize("DND5A.AttackPl"), items: [], hasActions: true,
        dataset: {type: "weapon", "weapon-type": "natural"} },
      actions: { label: game.i18n.localize("DND5A.ActionPl"), items: [], hasActions: true,
        dataset: {type: "feat", "activation.type": "action"} },
      passive: { label: game.i18n.localize("DND5A.Features"), items: [], dataset: {type: "feat"} },
      equipment: { label: game.i18n.localize("DND5A.Inventory"), items: [], dataset: {type: "loot"}}
    };

    // Start by classifying items into groups for rendering
    const maxLevelDelta = CONFIG.DND5A.maxLevel - (this.actor.system.details.level ?? 0);
    const [spells, other] = context.items.reduce((arr, item) => {
      ...
      if ( item.type === "class" ) ctx.availableLevels = Array.fromRange(CONFIG.DND5A.maxLevel, 1).map(level => ({
  ...
  getArmorLabel() {
    ...
    const label = [];
    if ( ac.calc === "default" ) label.push(this.actor.armor?.name || game.i18n.localize("DND5A.ArmorClassUnarmored"));
    else label.push(game.i18n.localize(CONFIG.DND5A.armorClasses[ac.calc].label));
