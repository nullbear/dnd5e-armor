...
export default class JournalClassPageSheet extends JournalPageSheet {
  ...
  /** @inheritdoc */
  get template() {
    return `systems/dnd5a/templates/journal/page-${this.document.type}-${this.isEditable ? "edit" : "view"}.hbs`;
  }
  ...
  async _getTable(item, initialLevel=1) {
    ...
    const headers = [[{content: game.i18n.localize("DND5A.Level")}]];
    if ( item.type === "class" ) headers[0].push({content: game.i18n.localize("DND5A.ProficiencyBonus")});
    if ( hasFeatures ) headers[0].push({content: game.i18n.localize("DND5A.Features")});
    ...
    const rows = [];
    for ( const level of Array.fromRange((CONFIG.DND5A.maxLevel - (initialLevel - 1)), initialLevel) ) {
      const features = [];
      for ( const advancement of item.advancement.byLevel[level] ) {
        switch ( advancement.constructor.typeName ) {
          case "AbilityScoreImprovement":
            features.push(game.i18n.localize("DND5A.AdvancementAbilityScoreImprovementTitle"));
            continue;
          case "ItemGrant":
  ...
  async _getSpellProgression(item) {
    ...
    if ( spellcasting.type === "leveled" ) {
      const spells = {};
      const maxSpellLevel = CONFIG.DND5A.SPELL_SLOT_TABLE[CONFIG.DND5A.SPELL_SLOT_TABLE.length - 1].length;
      Array.fromRange(maxSpellLevel, 1).forEach(l => spells[`spell${l}`] = {});

      let largestSlot;
      for ( const level of Array.fromRange(CONFIG.DND5A.maxLevel, 1).reverse() ) {
      ...
      table.headers = [
        [{content: game.i18n.localize("JOURNALENTRYPAGE.DND5A.Class.SpellSlotsPerSpellLevel"), colSpan: largestSlot}],
      ...
      table.headers = [[
        { content: game.i18n.localize("JOURNALENTRYPAGE.DND5A.Class.SpellSlots") },
        { content: game.i18n.localize("JOURNALENTRYPAGE.DND5A.Class.SpellSlotLevel") }
      ]];
      table.cols = [{class: "spellcasting", span: 2}];

      // Loop through each level, gathering "Spell Slots" & "Slot Level" for each one
      for ( const level of Array.fromRange(CONFIG.DND5A.maxLevel, 1) ) {
       ...
       * The actual hook names include the spellcasting type (e.g. `dnd5a.buildPsionicSpellcastingTable`).
       ...
       * @function dnd5a.buildSpellcastingTable
       * @memberof hookEvents
       */
      Hooks.callAll(
        `dnd5a.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting
      );
    }
  ...
  async _getOptionalTable(item) {
    const headers = [[
      { content: game.i18n.localize("DND5A.Level") },
      { content: game.i18n.localize("DND5A.Features") }
    ]];
