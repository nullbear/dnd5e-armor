...
export class ModuleArt {
  ...
  async #parseArtMapping(moduleId, mapping, credit) {
    let settings = game.settings.get("dnd5a", "moduleArtConfiguration")?.[moduleId];
  ...
  static getArtModules() {
    const settings = game.settings.get("dnd5a", "moduleArtConfiguration");
    const unsorted = [];
    const configs = [{
      id: game.system.id,
      label: game.system.title,
      mapping: "systems/dnd5a/json/fa-token-mapping.json",
      priority: settings.dnd5a?.priority ?? CONST.SORT_INTEGER_DENSITY,
      credit: `
        <em>
          Token artwork by
    ...
    for ( const module of game.modules ) {
      ...
      if ( !mapping ) continue;
      const config = { id: module.id, label: module.title, credit: flags?.["dnd5a-art-credit"], mapping };
      configs.push(config);
...
export class ModuleArtConfig extends FormApplication {
  ...
  constructor(object={}, options={}) {
    object = foundry.utils.mergeObject(game.settings.get("dnd5a", "moduleArtConfiguration"), object, {inplace: false});
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      title: game.i18n.localize("DND5A.ModuleArtConfigL"),
      id: "module-art-config",
      template: "systems/dnd5a/templates/apps/module-art-config.hbs",
      popOut: true,
      width: 600,
      height: "auto"
    });
  ...
  async _updateObject(event, formData) {
    await game.settings.set("dnd5a", "moduleArtConfiguration", foundry.utils.expandObject(formData));
